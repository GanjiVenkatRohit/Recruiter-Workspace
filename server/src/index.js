...
app.post('/api/uploads/:sessionId/complete', requireAuth, async (req, res) => {
  try {
    ...
    // Concatenate chunks and clean up session directory
    const finalFilePath = await storage.completeUpload(sessionId, totalChunks, candidateId, fileName);
    const absoluteFilePath = path.resolve(__dirname, '..', finalFilePath);
    ...
    // this reads the file from disk and enqueues a background job).
    try {
      ...
      await emitEventDirect(
        'ResumeUploaded',
        candidateId,
        { uploadId: sessionId, candidateId, fileName, checksum },
        sessionId,
        null,
